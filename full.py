...
supplier_totals = (
    df.groupby("Supplier")["SIPRI TIV of delivered weapons"]
    .sum()
...
# Total value by year
yearly_totals = df.groupby(df["OrderYear"].dt.year)[
...
supplier_totals_filtered = (
    df_filtered.groupby("Supplier")["SIPRI TIV of delivered weapons"]
    .sum()
    .sort_values(ascending=False)
    .head(10)
)
...
ax.pie(
    supplier_totals_filtered,
    labels=supplier_totals_filtered.index,
    autopct="%1.1f%%",
    startangle=90,
    counterclock=False,
)
ax.set_title(
    f"Top 10 Supplier Countries by Total Weapons Value for {recipient_country}"
)
st.pyplot(fig)

# Total value by year for selected recipient
yearly_totals_filtered = df_filtered.groupby(df_filtered["OrderYear"].dt.year)[
    "SIPRI TIV of delivered weapons"
].sum()
fig, ax = plt.subplots()
...
df_heatmap_filtered = df_filtered[
    (df_filtered["OrderYear"].dt.year >= 2010)
    & (df_filtered["OrderYear"].dt.year <= 2023)
]
top_20_weapons = (
    df_heatmap_filtered.groupby("Weapon description")["Number delivered"]
    .sum()
    .sort_values(ascending=False)
    .head(20)
    .index
)
df_heatmap_filtered = df_heatmap_filtered[
    df_heatmap_filtered["Weapon description"].isin(top_20_weapons)
]

df_heatmap = df_heatmap_filtered.pivot_table(
    values="SIPRI TIV of delivered weapons",
    index="Weapon description",
    columns=df_heatmap_filtered["OrderYear"].dt.year,
    aggfunc="sum",
    fill_value=0,
)
...
ax.set_title(
    f"Heatmap of Total Weapons Value by Weapon Type and Year (2010-2023) for {recipient_country}"
)
ax.set_xlabel("Year")
